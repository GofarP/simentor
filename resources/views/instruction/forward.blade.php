<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-12 w-full mx-auto bg-gray-50 dark:bg-gray-900 min-h-screen">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow p-8 w-full">
            ...
            <form method="POST" action="{{ route('instruction.forwardinstruction',$instruction->id) }}">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <select name="forwarded_to[]" id="forwarded_to"
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ old('forwarded_to', $instruction->forwarded_to) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
