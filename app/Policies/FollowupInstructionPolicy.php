<?php

namespace App\Policies;

use App\Models\FollowupInstruction;
use App\Models\User;
use Illuminate\Auth\Access\Response;

class FollowupInstructionPolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        return false;
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, FollowupInstruction $followupInstruction): bool
    {
        return $user->id === $followupInstruction->sender_id
            || $user->id === $followupInstruction->receiver_id;
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user, FollowupInstruction $followup): bool
    {
        return $user->id === $followup->receiver_id;
    }

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, FollowupInstruction $followupInstruction): bool
    {
        return $user->id === $followupInstruction->sender_id;
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, FollowupInstruction $followupInstruction): bool
    {
        return $user->id === $followupInstruction->sender_id;;
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, FollowupInstruction $followupInstruction): bool
    {
        return false;
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, FollowupInstruction $followupInstruction): bool
    {
        return false;
    }

    public function forward(User $user, FollowupInstruction $followupInstruction)
    {
        if ($user->hasRole('kasubbag') && $user->id == $followupInstruction->sender_id) {
            return false;
        }

        return (
            $user->id == $followupInstruction->sender_id ||
            $user->hasRole('kasubbag')
        );
    }
}
